...
val b = Json.Bool(false)
b

val n = Json.Num(5)


Json.Bool(false).value
Json.Num(5).value
n.ordinal
...
trait DecoderInstances:
  ...
  given Decoder[Int] = 
    Decoder.fromFunction {
        case Json.Num(n) if n.isValidInt => Some(n.toInt)
        case _ => None
    }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] =
      Decoder.fromFunction {
        case Json.Arr(items:List[Json]) => 
            val decodeResult = items.map(decoder.decode(_))
            Some(decodeResult.filter(_.isDefined).map(_.get))
        case _ => None
      }
    /*
      given [A] (using encoder: Encoder[A]): Encoder[List[A]] = 
        Encoder.fromFunction(as => Json.Arr(as.map(encoder.encode))) 
    */
    //Decoder.fromFunction(jsnArr => decoder.decode(jsnArr))
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] = 
      ...
      .transform[Contacts](ppl => ppl.people)

  given Decoder[Contacts] =
      Decoder.field[List[Person]]("people")
      .transform[Contacts](ppl => Contacts(ppl))
