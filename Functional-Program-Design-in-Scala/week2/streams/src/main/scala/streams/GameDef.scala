...
trait GameDef:
  ...
  case class Block(b1: Pos, b2: Pos):
    ...
    def neighbors: List[(Block, Move)] =
      def iter(mvList: List[Move]): List[(Block, Move)] = mvList match
        case Nil => Nil
        case head :: tail => head match
          case Move.Left => List((this.left, Move.Left)) ++ iter(tail)
          case Move.Right => List((this.right, Move.Right))  ++ iter(tail)
          case Move.Up => List((this.up, Move.Up)) ++ iter(tail)
          case Move.Down => List((this.down, Move.Down)) ++ iter(tail)
